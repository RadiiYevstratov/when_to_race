...
class SourceRegistryTests(unittest.TestCase):
    ...
    def test_imsa_has_an_adapter_and_still_does_not_run(self):
        """A finished adapter is not the same as one that can run.

        IMSA's is written and tested and has never fetched a page in anger:
        imsa.com answers 403 to this project's HTTP client while serving the
        identical URL to Python's standard library. The block ignores the
        User-Agent, so it is not the site refusing us by name - but getting
        past it would mean changing which client appears to be asking, and
        rule 3 in docs/sources.md is to be a good citizen visibly.

        Pinned because "the adapter is right there" is exactly the reason
        someone later turns it on without finding out why it was off.
        """
        registry = load_series()
        self.assertIn("imsa", registered_adapters())
        self.assertEqual(registry["imsa"].source.status, "unverified")
        self.assertFalse(registry["imsa"].source.is_verified)
